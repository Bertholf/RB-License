...
// Admin Head Section 
	...
	add_action('admin_head', 'rb_license_admin_head');
		function rb_license_admin_head(){
		  if( is_admin() ) {
			  
			// Add Stylesheet to WordPress Admin
			echo "<link rel=\"stylesheet\" href=\"". rb_license_BASEDIR ."style/admin.css\" type=\"text/css\" media=\"screen\" />\n";
...
// Page Head Section
	...
	add_action('wp_head', 'rb_license_inserthead');
		// Call Custom Code to put in header
		function rb_license_inserthead() {
		  if( !is_admin() ) {
...
// Add to WordPress Dashboard
	...
	add_action('wp_dashboard_setup', 'rb_license_add_dashboard' );
		...
		function rb_license_add_dashboard() {
			...
			$rb_license_dashboard_merge_array = array("rb_license_dashboard_quicklinks" => $left_dashboard["rb_license_dashboard_quicklinks"]);
			
			unset($left_dashboard['rb_license_dashboard_quicklinks']); // remove the new widget from the left column
			$right_dashboard = array_merge($rb_license_dashboard_merge_array, $right_dashboard); // use array_merge so that the new widget is pushed on to the beginning of the right column's array  
		...
		function rb_license_dashboard_quicklinks() {
			// Display Quicklinks
			$rb_license_options_arr = get_option('rb_license_options');
			if (isset($rb_license_options_arr['dashboardQuickLinks'])) {
			  echo $rb_license_options_arr['dashboardQuickLinks'];
			}
			...
			// Checks that the object is created correctly 
				...
				if ($maxitems == 0) {
				  ...
				  foreach ( $rss_items as $item ) {
					...
					echo "    <h4><a href='". $item->get_permalink() ."' title='Posted ". $item->get_date('j F Y | g:i a') ."' target=\"_license\">". $item->get_title() ."</a></h4>\n";
...
// Handle Folders
	...
	// Adding a new rule
	add_filter('rewrite_rules_array','rb_license_rewriteRules');
		function rb_license_rewriteRules($rules) {
			$newrules = array();
			// Pagination
			$newrules['license-category/(.*)/([0-9])$'] = 'index.php?type=category&target=$matches[1]&paging=$matches[2]';
			// Type
	...
	add_action( 'query_vars', 'rb_license_query_vars' );
		function rb_license_query_vars( $query_vars ) {
			$query_vars[] = 'type';
			$query_vars[] = 'target';
			$query_vars[] = 'paging';
			$query_vars[] = 'value';
	...
	add_filter('init','rb_license_flushrules');
		function rb_license_flushRules() {
			global $wp_rewrite;
...
// General Functions
	...
	function rb_license_cleanString($string) {
	...
	function rb_license_getActiveLanguage() {
	...
	function rb_license_random() {
		return preg_replace("/([0-9])/e","chr((\\1+112))",rand(100000,999999));
	}
	
	function rb_license_get_userrole() {
		global $current_user;
		get_currentuserinfo();
	...
	function rb_license_convertdatetime($datetime) {
		// Convert
	...
	function rb_license_makeago($timestamp, $offset){
	...
	function rb_license_get_age($p_strDate) {
		list($Y,$m,$d) = explode("-",$p_strDate);
		return( date("md") < $m.$d ? date("Y")-$Y-1 : date("Y")-$Y );
	}
	
	function rb_license_collapseWhiteSpace($string) {
		return preg_replace('/\s+/', ' ', $string);
	}
	
	function rb_license_safename($filename) {
		$filename = rb_license_collapseWhiteSpace(trim($filename));
	...
	function rb_license_filenameextension($filename) {
	...
	function rb_license_strtoproper($someString) {
...
// Shortcodes
	...
	/****************  List Records  ***************/
	// 
	function rb_license_list($atts, $content = NULL) {
		/* Example usage:
		if (function_exists('rb_license_list')) { 
		...
		*/
			...
			// Get Preferences
			$rb_license_options_arr = get_option('rb_license_options');
				$rb_license_option_name1 = $rb_license_options_arr['rb_license_option_name1'];  // Use (int)$rb_license_options_arr['rb_license_option_name1']  if integer
				$rb_license_option_name2 = $rb_license_options_arr['rb_license_option_name2'];
			...
			if (!isset($pagingperpage) || empty($pagingperpage)) { $pagingperpage = $rb_license_option_name2; } 
	

			/***********  Paginate  **************/

			$items = mysql_num_rows(mysql_query("SELECT LicenseTitle FROM  ". table_license_license ." WHERE LicenseActive = 1 $filter")); // number of total rows in the database
			if($items > 0) {
				$p = new rb_license_pagination;
				$p->items($items);
			...
			/*********** Execute Query **************/
			// Query
			$queryList = "SELECT LicenseTitle FROM  ". table_license_license ." WHERE LicenseActive = 1 $filter ORDER BY LicenseDateCreated ASC";
			...
			// Pages
			if($items > 0) {
				echo "    <div class=\"license-results-pages\">\n";
				echo 		$p->show();  // Echo out the list of paging. 
				echo "    </div>\n";
			}
			// Count
				echo "    <div class=\"license-results-count\">\n";
				echo "    	". __("Displaying", rb_license_TEXTDOMAIN) ." <strong>". $countList ."</strong> ". __("of", rb_license_TEXTDOMAIN) ." ". $items ." ". __(" records", rb_license_TEXTDOMAIN) ."\n";
				echo "    </div>\n";


			/*********** Display List **************/
			echo "<div id=\"license-list\">\n";
			while ($dataList = mysql_fetch_array($resultsList)) {
				if ($type == true) {
					// This is a typ
					echo "  <div class=\"layout1\">". $dataList["LicenseTitle"] ."</div>\n";
				} else {
					// This is a different type
					echo "  <div class=\"layout2\">". $dataList["LicenseTitle"] ."</div>\n";
				}
			}
			if ($countList < 1) {
				echo __("No Profiles Found", rb_license_TEXTDOMAIN);
			}
			echo "</div>\n";
	...
	/****************  Display Record  ***************/
	// 
	function rb_license_display($atts, $content = NULL) {
		/* Example usage:
		if (function_exists('rb_license_list')) { 
			$atts = array('recordid' => 1, 'type' => 'Value One');
			rb_license_list($atts); }
		*/
		...
		extract(shortcode_atts(array(
			...
			if (isset($recordid) && !empty($recordid)){
				$filter .= " AND LicenseAutoID='". $recordid ."'";
			}
